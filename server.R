...
shinyServer(function(input, output, session) {
  ...
  output$shapiroTest <- renderValueBox({
    data <- data.frame(helloKittyData())
    
    shapiroResult <- shapiro.test(data$PKS)
    ShapiroPValue <- shapiroResult$p.value
    print(shapiroResult$p.value)
    print(shapiroResult$statistic)
    getLog <- log(data$PKS,10)
    print('get log ',getLog)
    
    valueBox(ShapiroPValue,"Shapiro Result", 
             color = 'light-blue') 
